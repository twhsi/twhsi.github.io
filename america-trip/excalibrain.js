...
function pickHubPath(note, usedPaths) {
  const backlinks = note.backlinks
    .map((path) => state.data.notes.find((item) => item.path === path))
    .filter(Boolean);
  const preferred = backlinks.find((item) => /美國之旅|usa\s*trip/i.test(`${item.title} ${item.basename} ${item.path}`));
  if (preferred && !usedPaths.has(preferred.path) && preferred.path !== note.path) return preferred.path;

  const fallback = state.data.notes.find((item) => /美國之旅|usa\s*trip/i.test(`${item.title} ${item.basename} ${item.path}`));
...
function buildExcalibrainGraph(note) {
  ...
  const nodes = [centerNode, ...filteredParents, ...filteredChildren, ...filteredPrev, ...filteredNext];
...
function renderExcalibrainStatus(note) {
  ...
  const properties = ["Parent", "Child", "Prev", "Next"].filter((key) => relationNotePaths(note, key).length > 0).length;
  ...
  excalStatusEl.textContent = `${note.backlinks.length} backlinks  ${properties} properties  ${words} words  ${chars.toLocaleString("en-US")} characters`;
}
